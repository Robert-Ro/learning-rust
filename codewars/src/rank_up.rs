...
pub fn anagrams(word: &str, words: &[String]) -> Vec<String> {
    words
        ...
        .into_iter()
        .filter(|x| {
            let mut x_s = x.as_bytes().to_vec();
            x_s.sort();
            let mut w_s = word.as_bytes().to_vec();
            w_s.sort();
            x_s == w_s
        })
        .collect()
...
fn sample_tests() {
    ...
    assert_eq!(persistence2(39), 3);
    assert_eq!(persistence2(4), 0);
    assert_eq!(persistence2(25), 2);
    assert_eq!(persistence2(999), 4);
}
#[test]
